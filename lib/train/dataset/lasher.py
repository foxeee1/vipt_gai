...
class LasHeR(BaseVideoDataset):
    ...
    def _extract_frame_number(self, filename):
        """从文件名中提取数字编号（处理如 v0.jpg、v0001.jpg 等格式）"""
        match = re.search(r'v(\d+)\.jpg', filename, re.IGNORECASE)
        if match:
            return int(match.group(1))
        else:
            # 备用逻辑：提取文件名中所有数字部分
            num_part = ''.join(filter(str.isdigit, filename.split('.')[0]))
            return int(num_part) if num_part else 0  # 无数字时返回0（避免索引错误）

    def _get_frame_path(self, seq_path, frame_id):
        vis_dir = os.path.join(seq_path, 'visible')
        inf_dir = os.path.join(seq_path, 'infrared')

        # 筛选出 .jpg 格式的帧文件
        vis_frame_names = [f for f in os.listdir(vis_dir) if f.lower().endswith('.jpg')]
        inf_frame_names = [f for f in os.listdir(inf_dir) if f.lower().endswith('.jpg')]

        # 按帧号排序（确保不同命名格式的文件按数字顺序排列）
        vis_frame_names.sort(key=lambda x: self._extract_frame_number(x))
        inf_frame_names.sort(key=lambda x: self._extract_frame_number(x))

        # 索引越界检查（可选，用于调试）
        if frame_id >= len(vis_frame_names) or frame_id >= len(inf_frame_names):
            raise IndexError(
                f"frame_id {frame_id} 越界！visible 有 {len(vis_frame_names)} 帧，infrared 有 {len(inf_frame_names)} 帧。")

        return (
            os.path.join(vis_dir, vis_frame_names[frame_id]),
            os.path.join(inf_dir, inf_frame_names[frame_id])
        )

    def _get_frame(self, seq_path, frame_id):
        vis_path, inf_path = self._get_frame_path(seq_path, frame_id)
        # 读取可见光和红外图像并拼接
        vis_img = self.image_loader(vis_path)
        inf_img = self.image_loader(inf_path)
        return np.concatenate((vis_img, inf_img), axis=2)
