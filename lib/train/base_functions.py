...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        # 添加大小写不敏感的数据集名称支持
        name_mapping = {
            'lasher_train': 'LasHeR_train',
            'lasher_val': 'LasHeR_val',
            'lasher_all': 'LasHeR_all',
            'depthtrack_train': 'DepthTrack_train',
            'depthtrack_val': 'DepthTrack_val',
        }
        if name.lower() in name_mapping:
            name = name_mapping[name.lower()]
...
def get_optimizer_scheduler(net, cfg):
    ...
    if cfg.TRAIN.SCHEDULER.TYPE == 'step':
        ...
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=cfg.TRAIN.EPOCH, eta_min=getattr(cfg.TRAIN.SCHEDULER, 'ETA_MIN', 1e-6))
    else:
        # lr_scheduler = None
