...
class LTRTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, use_amp=False):
        ...
        if settings.local_rank in [-1, 0]:
            ...
            # 初始化Prompt可视化器（兼容cfg未加载的情况）
            try:
                enable_vis = getattr(settings.cfg.MODEL.BACKBONE, 'ENABLE_VIS', False)
                vis_interval = getattr(settings.cfg.MODEL.BACKBONE, 'VIS_LOG_INTERVAL', 50)
            except AttributeError:
                enable_vis = False
                vis_interval = 50
            PromptVisualizer.setup(log_dir=tensorboard_writer_dir, enabled=enable_vis, log_interval=vis_interval)
