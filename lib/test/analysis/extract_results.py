...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            # Load results
            # 生成包含tracker_param和dataset的完整路径
            base_results_path = os.path.join(trk.results_dir, trk.parameter_name, seq.dataset, seq.name)
            results_path = os.path.join(base_results_path + '.txt')
