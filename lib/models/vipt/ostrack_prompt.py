...
def build_viptrack(cfg, training=True):
    ...
    consistency_version = getattr(cfg.MODEL.BACKBONE, 'CONSISTENCY_VERSION', 'fusion')
    
    # 读取灵活注入层数配置
    base_prompt_inject_layers = getattr(cfg.MODEL.BACKBONE, 'BASE_PROMPT_INJECT_LAYERS', None)
    meta_prompt_inject_layers = getattr(cfg.MODEL.BACKBONE, 'META_PROMPT_INJECT_LAYERS', None)
    ...
    if cfg.MODEL.BACKBONE.TYPE == 'vit_base_patch16_224_prompt':
        ...
        if meta_prompt:
            backbone = vit_base_patch16_224_ce_prompt(
                ...
                meta_prompt=True,
                consistency_version=consistency_version,
                base_prompt_inject_layers=base_prompt_inject_layers,
                meta_prompt_inject_layers=meta_prompt_inject_layers,
                meta_enable_mask=cfg.MODEL.BACKBONE.get('META_ENABLE_MASK', True),
                meta_enable_temporal=cfg.MODEL.BACKBONE.get('META_ENABLE_TEMPORAL', True),
