...
class VisionTransformerP(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # 元提示生成器
        if self.meta_prompt:
            self.meta_prompt_generator = MetaPromptGenerator(embed_dim, consistency_version=consistency_version)
            # 记录历史特征
    ...
    def forward_features(self, z, x, mask_z=None, mask_x=None,
        ...
        if self.meta_prompt:
            # 保存原始特征用于元提示生成
            rgb_features = x[:, lens_z:, :]  # 搜索区域RGB特征
            x_features = x_dte  # X模态特征
        ...
        for i, blk in enumerate(self.blocks):
            ...
            # 注入元提示（高层，第9-12层）
            if self.meta_prompt and i >= 8:  # 第9层开始（索引8）
                # 生成元提示
                meta_prompt = self.meta_prompt_generator(rgb_features, x_features, self.prev_features)
                # 将元提示拼接到特征中
                x = torch.cat([x, meta_prompt], dim=1)
